...
3. Deploy. The start command runs `prisma migrate deploy` before `next
   start`, so the schema is applied automatically on first boot — no
   separate migration step needed against the deployed DB.
